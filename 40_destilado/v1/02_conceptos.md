...
- **Límite:** Price Swing no define de forma cerrada las `breakdown characteristics`, hard stop ni todos los parámetros de reentrada. [SOLO-PS: no confirmado después] como nombre/forma; [PRECURSOR: PS → refinado en M2020] por drained shelf, parabolic/technical break y S-08. No equivale a toda continuation leg ni a una PI genérica.
...
- **Lectura del primer break (S13):** cuando el precio rompe midflow, antes de continuar a nuevos extremos "**it looks like an apex anyway**" — prohibido tener expectativas rígidas; ampliar el reference point con variaciones (bayesiano). El primer break de shelf/midflow **típicamente golpea un nivel residual**, no un área fuerte — eso ayuda a decidir reversal vs continuación. Y el **tipo de nivel que golpeó el extremo** antes de romper shelf pondera si el high/low aguanta: residual → no esperar que aguante; core/buen nivel → mayor probabilidad. [VID-M2020: Session 13 @ 01:26:45–01:29:49]
...
- La serie etiqueta la zona como `Fractal IV`, `IV Consumed` y `Washed IV`. EM explica: "**Price consuming the inventory, weakening the area. Goes deep into it, better yet it pokes past it**". [IMG: CLP/ED6JtkLXsAEu4X4.png] [IMG: CLP/EEBYb6-WsAAp2N-.png] [IMG: CLP/EEBe34cXoAEhKWw.png] [IMG: CLP/EUR CPL.jpg]
- **[CONFIRMADO] `IV` = inventory** (elevado de inferencia el 2026-08-16): evidencia directa en la anotación 4 de la misma captura — "Bids dry off, **red inventory** pushes price back down through the rip area" — refiriéndose a la misma caja que el resto de la serie rotula `IV`. Ni volatilidad implícita ni numeral cuatro. La expansión LITERAL de las letras sigue sin aparecer escrita. [IMG: CLP/ED6JtkLXsAEu4X4.png]
...
- **Estado:** confirmado; expansión escrita por EM con flecha propia: `CPL - consumption response leg`. [IMG: CLP/ED6JtkLXsAEu4X4.png]
- ⚠️ **ANOMALÍA INTERNA DE LA FUENTE (sin resolver):** las siglas no cuadran con la expansión escrita — "consumption response leg" daría CRL, no CPL. Puede que EM omitiera una palabra o que CPL sea "Consumption Price Leg" y glosara de memoria. NO se infiere cuál; se registra tal cual (ver auditoria_corpus_v1.md).
...
- **Definición general — direccionalidad simétrica (2026-08-16):** la CPL es **la pierna que se aleja del inventario consumido**, en cualquier dirección; el trade fadea esa pierna con objetivo ATRAVESANDO la zona consumida hasta el siguiente inventario grueso. El setup NO es direccionalmente bajista por naturaleza: en [IMG: CLP/ED6JtkLXsAEu4X4.png] la CPL es alcista → short que rasga hacia abajo; en [IMG: CLP/EUR CPL.jpg] la CPL es bajista (~1.117→1.07778) → el precio rasga hacia arriba. Misma estructura en espejo.
- Visualmente aparece en XAUUSD t5/t30 (gráficos de 5 y 30 ticks)/1m/5m, GBPUSD 5m/D1 y EURUSD D1, a ambos lados del mercado; la fuente la presenta como fractal, no ligada a un instrumento/TF. **Fractalidad en cuatro escalas** (t30 ticks → 5min → 5min → D1 de 5 meses): la demostración fractal mejor documentada del corpus para un setup concreto. [IMG: CLP/EEBe34cXoAEhKWw.png] [IMG: CLP/EFyeKT-XkAAP053.png] [IMG: CLP/EEBnCEJWkAMJEdf.png] [IMG: CLP/EUR CPL.jpg] [IMG: CLP/EEBYb6-WsAAp2N-.png] [IMG: CLP/EEBaBCGXkAIIhpc.png] [IMG: CLP/EEBgtXpXUAAoEmR.png] [IMG: CLP/EEBhSR1XsAAh-uH.png]
